...
let currentDeterminant = null; // Store the current determinant
...
function computeDeterminant(A) {
    ...
    let det = 1;
    let swapCount = 0; // Track number of row swaps

    // Perform Gaussian elimination with partial pivoting
    for (let col = 0; col < n; col++) {
        // Find pivot
        const pivotRow = findPivot(matrix, col, col, 1e-10);

        if (pivotRow === -1) {
            // No pivot found, determinant is zero (singular matrix)
            return 0;
        }
        ...
        // If pivot is zero (within tolerance), matrix is singular
        if (isZero(pivot, 1e-10)) {
            return 0;
        }

        // Multiply determinant by the pivot
        det *= pivot;
    ...
    // Account for row swaps (each swap changes sign of determinant)
    if (swapCount % 2 === 1) {
        det = -det;
    }
    ...
    // Round to integer if very close
    return roundToInteger(det, 1e-9);
}
...
function displayResult(inverse) {
    ...
    if (inverse === null) {
        let html = '<div class="error-message">The matrix is Singular.</div>';

        // Show determinant = 0 for singular matrices
        if (currentDeterminant !== null) {
            html += '<div style="background: linear-gradient(135deg, rgba(220, 53, 69, 0.08) 0%, rgba(220, 53, 69, 0.05) 100%); padding: 15px 20px; border-radius: 10px; margin-top: 20px; border-left: 4px solid #dc3545;">';
            html += '<h4 style="margin: 0 0 8px 0; color: #dc3545; font-size: 1.05em;">Determinant (det A):</h4>';
            html += '<div style="font-size: 1.3em; font-weight: 700; color: #721c24;">';
            html += '0';
            html += '</div>';
            html += '<p style="margin: 10px 0 0 0; color: #721c24; font-size: 0.9em; font-style: italic;">A matrix with determinant 0 is not invertible.</p>';
            html += '</div>';
        }
...
function renderResult() {
    ...
    // Display determinant if available
    if (currentDeterminant !== null) {
        html += '<div style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.08) 0%, rgba(37, 99, 235, 0.08) 100%); padding: 15px 20px; border-radius: 10px; margin-bottom: 20px; border-left: 4px solid #3b82f6;">';
        html += '<h4 style="margin: 0 0 8px 0; color: #3b82f6; font-size: 1.05em;">Determinant (det A):</h4>';
        html += '<div style="font-size: 1.3em; font-weight: 700; color: #2d3748;">';
        html += formatNumber(currentDeterminant, displayFormat);
        html += '</div>';
        html += '</div>';
    }
